...
\begin{itemize}
  ...
  \item \textbf{Model comparison}: the full and the reduced models have been compared by using leave-one-out cross-validation and the best model has been selected for the remaining analysis; 
  ...
  \item \textbf{Age prediction}: to conclude we decided to choose one of the most influential parameters to generate a prediction and anser our analysis problem.
\end{itemize}
...
dens.beta <- stan_dens(model.full.beta, pars = c('as.numeric(Urban)', 'as.numeric(Sex)', 'as.numeric(Age)', 'as.numeric(Education)', 'as.numeric(Occupation)'))
dens.beta + ggtitle('Plots for beta prior')
...
trace.beta <- stan_trace(model.full.beta, pars = c('as.numeric(Urban)', 'as.numeric(Sex)', 'as.numeric(Age)', 'as.numeric(Education)', 'as.numeric(Occupation)'))
trace.beta + scale_color_brewer(type = 'div') + ggtitle('Plots for beta prior')
...
rhat.beta <- stan_rhat(model.full.beta, bins = 50)
...
beta <- data.frame(rhat = rhat.beta$data$stat)
...
distrLength <- rbind(uniform, normal, student, cauchy, beta)
...
ess.beta <- stan_ess(model.full.beta, bins = 50)
...
beta <- data.frame(ratio_ess = ess.beta$data$stat)
...
loo.beta <- loo(model.full.beta, cores = 4)
compar_models_prior <- loo_compare(loo.uniform, loo.normal, loo.student, loo.cauchy, loo.beta)
compar_models_prior
...
ppc_dens_overlay(as.numeric(died), y_tilde[1:50, ])
```
